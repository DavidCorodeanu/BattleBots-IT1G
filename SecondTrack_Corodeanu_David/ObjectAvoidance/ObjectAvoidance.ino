const int TRIG_PIN = 13;  // Digital pin for the distance sensor
const int ECHO_PIN = 12;  // Digital pin for the distance sensor
const int MOTOR_A1 = 11;  // PWM pin for the motor
const int MOTOR_A2 = 10;  // PWM pin for the motor
const int MOTOR_B1 = 9;   // PWM pin for the motor
const int MOTOR_B2 = 3;   // PWM pin for the motor

void setup() {
  // Initialize distance sensor pins
  pinMode(TRIG_PIN, OUTPUT);
  digitalWrite(TRIG_PIN, HIGH); // default off
  pinMode(ECHO_PIN, INPUT);
  digitalWrite(ECHO_PIN, HIGH); // default off
  // Initialize motor pins
  pinMode(MOTOR_A1, OUTPUT);
  digitalWrite(MOTOR_A1, HIGH); // default off
  pinMode(MOTOR_A2, OUTPUT);
  digitalWrite(MOTOR_A2, HIGH); // default off
  pinMode(MOTOR_B1, OUTPUT);
  digitalWrite(MOTOR_B1, HIGH); // default off
  pinMode(MOTOR_B2, OUTPUT);
  digitalWrite(MOTOR_B2, HIGH); // default off
